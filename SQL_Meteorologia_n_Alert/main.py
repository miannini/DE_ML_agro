...
resu = datab.groupby(['ID_Estacion','dif']).max(['FECHA_HORA']) #.iloc[:,0:2]
resu['mail']=0
resu.reset_index(inplace=True)

resu['dif'] = (datetime.strptime(max_date,'%Y-%m-%d') - resu['FECHA_HORA']).dt.days
...
for n in range(len(resu)):  
    if resu['mail'][n]==1:
        if resu['ID_FINCA'][n]==1:
            alpha ="1 - Juan Camilo"
            email(alpha, resu['FECHA_HORA'][n])
        elif resu['ID_FINCA'][n]==2:
