...
''' # NEVER USED
### List deeper blobs into defined bucket-folder
def list_deeper_blobs(storage_client, bucket_name, prefix=None, delimiter=None, bands=None): #prefix is initial route, delimiter is '/', bands
    lista = []
    #storage_client = storage.Client()
    blobs = storage_client.list_blobs(bucket_name, prefix=prefix,delimiter=delimiter)
    for blob in blobs:
        for banda in bands: #si ha restriccion de lote
            if re.search(r'_'+re.escape(banda)+r'_',blob.name):
                tmp_name = blob.name#.split('_')[0:-1] + '.jp2' #removes _$.folder$
                lista.append(tmp_name) 
    return lista
'''
'''
### Download blobs by name, defining destination name
...
'''
#additional filters to get final images / not used yet
gran='GRANULE'
im='IMG_DATA'
ext = '.jp2'
'''
...
### list of unique dates based on folder/file names
folds_create=[]        
for fil in files_fin:
    tmp_date = fil.split('/')[-1].split('_')[1][:8]
    if tmp_date not in folds_create: folds_create.append(tmp_date)
    
    
### for each unique folder create the folder if doesnt exist
for fold in folds_create:
    Path('../' + fold + '/').mkdir(parents=True, exist_ok=True)
              
### Download each file, in the corresponding folder, and edit name to avoid resolution comment

for n in files_fin: #range(36,70):
    #               gcp_client,  bucket,  file,       one folder up,   dated_folder,                                                 split name, remove resolution, rejoin and add .jp2
    download_blob(storage_client,buck, n, "../"+ n.split('/')[-1].split('_')[1][:8] +"/"+os.path.basename('_'.join(n.split('/')[-1].split('_')[:-1])+'.jp2'))
'''
...
#sent do SQL
#dates_df.to_sql('monitoreo_descargas_sentinel', engine,  if_exists='append', index=False)   

#correr funcion for n in dataframe (zona, cliente) = crop -> remocion nubes -> calculo indices -> crop lotes -> indices a tabla -> store images