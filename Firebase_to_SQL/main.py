...
def fire_to_gcp1(event,context):
    ...
    hist_db1 = db.child("iot").child("histo").child("Station1").get().val()
    hist_db2 = db.child("iot").child("histo").child("Station2").get().val()
    ...
    #format dates
    df_sta1["DateTime"] = pd.to_datetime(df_sta1["DateTime"], format='%Y-%m-%d %H:%M:%S') #datetime.datetime.strptime
    df_sta1.index = pd.to_datetime(df_sta1.index, format='%Y-%m-%d %H:%M:%S')
    df_sta2["Date_Time"] = pd.to_datetime(df_sta2["Date_Time"], format='%Y-%m-%d %H:%M:%S') #datetime.datetime.strptime
    df_sta2.index = pd.to_datetime(df_sta2.index, format='%Y-%m-%d %H:%M:%S')
    ...
    df_res1 = df_sta1.set_index('Date_Time').resample('D').agg({'DHT_Humidity':['mean','max','min','std'], 
