...
data = db.child("iot").child("histo").get().val()
data_len = len(db.child("iot").child("histo").get().val())
for n in range(data_len):
    print(n)
    hist_db = db.child("iot").child("histo").child("Station"+str(n+1)).get().val() #child
...
############################# process JSON data ##############################################3
    ...
    df_sta = pd.DataFrame.from_dict(hist_db, orient='index')
    #df_sta2 = pd.DataFrame.from_dict(hist_db2, orient='index')

    #format dates
    df_sta["Date_Time"] = pd.to_datetime(df_sta["Date_Time"], format='%Y-%m-%d %H:%M:%S') #datetime.datetime.strptime
    ...
    df_res = df_sta.set_index('Date_Time').resample('D').agg({'DHT_Humidity':['mean','max','min','std'], 
    ...
    #key ID values
    df_res['ID_Estacion'] = n+1
    #df_res['ID_FINCA'] = 1
    #df_res['ID_CLIENTE'] = 1
    #df_res2['ID_FINCA'] = 2
    #df_res2['ID_CLIENTE'] = 1

    data_all = data_all.append(df_res, sort=False)
